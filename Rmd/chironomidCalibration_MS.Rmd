...
All analyses were done in R version `r getRversion()` [@R2017]. Ordinations were fitted with vegan version `r packageVersion("vegan")` [@vegan2017] with square-root transformed assemblage data. Transfer functions were fitted with rioja version `r packageVersion("rioja")` [@rioja2017] using square-root transformed species data. Some diagnostics were performed using analogue version `r packageVersion("analogue")` [@analogue2016]. The reconstruction significance test was run with randomTF from palaeoSig version `r packageVersion("palaeoSig")` [@palaeoSig2015] with 999 trials for the null distribution.
...
readd(plot_performance_vs_range) %>% print()
```
